    ...
    if type == 1 or type == 2:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
            ...
            for i, bill_number in enumerate(dist_billnumber, start=1):
                ...
                future = executor.submit(process_data, bill_number, thread_name, i, url, exec_sql, exec_sql1, type)
        ...
        for billnumber in dist_billnumber:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
                ...
                futures = []
                # 遍历 dist_billnumber 列表，并为每个元素生成一个索引和对应的值。start=1 参数表示索引从 1 开始。
                for i, bill_number in enumerate(billnumber_detail, start=1):
                    # 构建线程名称
                    thread_name = "线程" + str(i % 5 + 1)
                    # 提交任务到线程池，并传递订单号、线程名称和计数
                    future = executor.submit(process_data, bill_number, thread_name, i, url, exec_sql, exec_sql1, type)
            ...
            for thread_name, count in thread_counts.items():
                write_log.write_log(f"线程 {thread_name} 执行了 {count} 次任务")
            write_log.write_log('对账单{}发送结束'.format(num))
            num = num + 1
...
def select_distno_detail(billnumber):
	...
	from  wy_trans_to_kingdee  a,e_account b,e_warehouse_org d
	where  a.bill_number=b.rec_bill_number
...
def process_data(bill_number, thread_name, i, url, exec_sql, exec_sql1, type):
    ...
    post_data(send_data, bill_number, i, thread_name, url)
...
def update_table(table_name, state, remark, billnumber):
    ...
    sql_str = """
    UPDATE  %s
    set state =%s,remark=%s,finish_time=now()
    where  bill_number=%s
    """
    try:
        cursor = cnx.cursor()
        cursor.execute(sql_str, (table_name, state, remark, billnumber))
        if cursor is None:
            write_log.write_log('{}: update{}连接数据库失败'.format(table_name, billnumber))
            dbcon.db_disconnect(cnx)  # 关闭数据库连接
            return False
